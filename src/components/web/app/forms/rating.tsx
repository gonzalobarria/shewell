...
interface RatingProps extends React.HTMLAttributes<HTMLDivElement> {
  ...
  onRatingChange?: ({ id, level }: { id: number; level: number }) => void
...
export const Rating = ({
  ...
  const handleClick = (event: React.MouseEvent<HTMLDivElement>) => {
    if (!disabled) {
      ...
      if (onRatingChange) {
        const tmpRating =
          currentRating > 0 && currentRating === starIndex ? 0 : starIndex

        setCurrentRating(tmpRating)
        onRatingChange({ id: itemId || 0, level: tmpRating })
      }
    }
  }
